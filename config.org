...
Since we're using use-package as our package management system, we might as well try to organize under the same syntax as much as possible to keep the configuration consistent. Since the option ~use-package-always-ensure~ is turned on in ~init.el~, we'll add ~:ensure nil~ when configuring the built-in packages.
